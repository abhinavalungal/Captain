'use strict';
/**
 * KRIS_ALLOW_ORIGIN accepts three shapes: an exact origin, "*" for any
...
t('exact origin match still works (today\'s behaviour, unchanged)', () => {
  const h = corsHeaders('https://perform.geoserves.com', { KRIS_ALLOW_ORIGIN: 'https://perform.geoserves.com' });
  assert.strictEqual(h['Access-Control-Allow-Origin'], 'https://perform.geoserves.com');
});

t('a non-listed exact origin is refused', () => {
  const h = corsHeaders('https://evil.example.com', { KRIS_ALLOW_ORIGIN: 'https://perform.geoserves.com' });
  assert.strictEqual(h['Access-Control-Allow-Origin'], undefined);
});

t('bare "*" allows anything, as before', () => {
  const h = corsHeaders('https://anything-at-all.example.org', { KRIS_ALLOW_ORIGIN: '*' });
  assert.strictEqual(h['Access-Control-Allow-Origin'], '*');
});

t('*.netlify.app matches a preview deploy subdomain', () => {
  const h = corsHeaders('https://deploy-preview-123--geoserve-fueleu.netlify.app', { KRIS_ALLOW_ORIGIN: '*.netlify.app' });
...
t('*.netlify.app matches the bare app domain too', () => {
  const h = corsHeaders('https://geoserve-fueleu.netlify.app', { KRIS_ALLOW_ORIGIN: '*.netlify.app' });
...
t('*.netlify.app does NOT match an unrelated domain', () => {
  const h = corsHeaders('https://netlify.app.evil.com', { KRIS_ALLOW_ORIGIN: '*.netlify.app' });
  assert.strictEqual(h['Access-Control-Allow-Origin'], undefined);
});

t('*.netlify.app does NOT match netlify.app being a substring elsewhere', () => {
  const h = corsHeaders('https://notnetlify.app', { KRIS_ALLOW_ORIGIN: '*.netlify.app' });
  assert.strictEqual(h['Access-Control-Allow-Origin'], undefined);
});

t('mixed list: literal domain + wildcard domain, both honoured', () => {
  const env = { KRIS_ALLOW_ORIGIN: 'https://perform.geoserves.com,*.netlify.app' };
...
t('a request with no Origin header (curl, health check) still works when "*" is set', () => {
  const h = corsHeaders('', { KRIS_ALLOW_ORIGIN: '*' });
  assert.strictEqual(h['Access-Control-Allow-Origin'], '*');
});

t('a request with no Origin header is fine even with a strict allowlist (no CORS headers needed for non-browser callers)', () => {
  const h = corsHeaders('', { KRIS_ALLOW_ORIGIN: 'https://perform.geoserves.com' });
  assert.strictEqual(h['Access-Control-Allow-Origin'], undefined);
});

t('a malformed Origin header does not crash the matcher', () => {
  const h = corsHeaders('not-a-valid-url', { KRIS_ALLOW_ORIGIN: '*.netlify.app' });
  assert.strictEqual(h['Access-Control-Allow-Origin'], undefined);
});

t('allowedOriginsList trims whitespace and drops empties, unchanged', () => {
  assert.deepStrictEqual(allowedOriginsList({ KRIS_ALLOW_ORIGIN: ' https://a.com , *.netlify.app ,' }), ['https://a.com', '*.netlify.app']);
});
