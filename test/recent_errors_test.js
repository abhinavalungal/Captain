...
(async()=>{
  ...
  await h.handleKris({method:'POST',headers:{authorization:'Bearer '+token},body:JSON.stringify({text:'what 22-11'}),env:{KRIS_DEV_SESSION:'1'}});
  router.route=orig;
  // 2. a DB connect failure -> recorded, secrets scrubbed
  await h.handleKris({method:'POST',headers:{authorization:'Bearer '+token},body:JSON.stringify({text:'fueleu penalty for STI ROTHERHITHE this year'}),env:{KRIS_DEV_SESSION:'1',KRIS_ENABLE_LLM:'0',KRIS_READ_URL:'postgresql://postgres.abc:Secret%40123@nowhere.invalid:5432/postgres',KRIS_PG_CONNECT_TIMEOUT_MS:'2000'}});
  const health=JSON.parse((await h.handleKris({method:'GET',headers:{},env:{KRIS_DEV_SESSION:'1'}})).body);
  ...
  const prod=JSON.parse((await h.handleKris({method:'GET',headers:{},env:{}})).body);
