'use strict';
/**
 * K.R.1.S tests — identity, name memory, unit conversion, follow-up
 * inheritance and their routing. Entirely offline: no database, no model.
 *
 *   node test/persona_test.js
 */
...
// --- identity ----------------------------------------------------------------
t('identity: "what is your name" introduces K.R.1.S and asks back', () => {
  const a = identity.answerIdentity("what's your name?", {});
  assert.ok(/K\.R\.1\.S/.test(a.text));
...
(async () => {
  ...
  await ta('router: "introduce yourself" goes to the guide, which now says K.R.1.S', async () => {
  ...
  // --- companion grounding ----------------------------------------------------------
  t('companion: system prompt names K.R.1.S and, when known, the user', () => {
    const p = systemPrompt({ appName: 'Shuddha now', guideSnippets: [], userName: 'Nav' });
    assert.ok(/You are K\.R\.1\.S/.test(p));
    assert.ok(/user's name is Nav/.test(p));
    const anon = systemPrompt({ appName: 'Shuddha now', guideSnippets: [] });
  ...
  await ta('companion: the model is told the user\'s name via context.userName', async () => {
    ...
    const spy = async (url, init) => {
      ...
      return { ok: true, status: 200, json: async () => ({ message: { role: 'assistant', content: 'Certainly.' } }) };
    };
    await router.route(
  ...
  await ta('http: context.userName survives the handler whitelist end to end', async () => {
    ...
    const r = await handler.handleKris({
      method: 'POST',
  ...
  await ta('"what can you do" gets the capability overview with next steps; "help" keeps the catalogue', async () => {
    ...
    assert.strictEqual(r.instant, true);
    assert.ok(/K\.R\.1\.S/.test(r.text), r.text);
  ...
  // --- the K.R.1.S identity ------------------------------------------------------
  t('persona: the codename is explained the same way however it is asked', () => {
    const asks = ['what does K.R.1.S stand for?', 'what does kris mean', 'why are you called K.R.1.S', 'meaning of your name', 'are you krishna?', 'Are you Lord Krishna'];
    const texts = asks.map((q) => { const a = identity.answerIdentity(q, {}); assert.ok(a, 'not answered: ' + q); assert.strictEqual(a.kind, 'kris_meaning', q); return a.text; });
    assert.ok(texts.every((x) => x === texts[0]), 'the explanation must be one fixed sentence');
    assert.ok(/inspired by Lord Krishna/.test(texts[0]) && /charioteer/.test(texts[0]) && /Kris/.test(texts[0]));
  });
  t('persona: ordinary "what does X mean" questions are not mistaken for the codename', () => {
    for (const q of ['what does FuelEU mean', 'what does CII stand for', 'why is my ship slow', 'who is the charterer']) {
      const a = identity.answerIdentity(q, {});
      assert.ok(!a || a.kind !== 'kris_meaning', 'wrongly matched: ' + q);
    }
  });
  await ta('persona: the codename answer is instant, from the fast lane, with no model or database', async () => {
    const out = await route('what does K.R.1.S stand for?');
    assert.strictEqual(out.source, 'identity');
    assert.ok(out.instant);
    assert.ok(/Lord Krishna/.test(out.text));
  });
  t('persona: a greeting the user chose is returned in kind, and only then', () => {
    const tz = { context: { tz: 'Asia/Calcutta' }, now: NOW };
    assert.ok(/^Namaste, Nav\./.test(router.smallTalkReply('namaste', 'Nav', tz)));
    assert.ok(/^Radhe Radhe\./.test(router.smallTalkReply('Radhe Radhe', null, tz)));
    assert.ok(/^Hare Krishna\./.test(router.smallTalkReply('hare krishna', null, tz)));
    assert.ok(/^Jai Shri Krishna\./.test(router.smallTalkReply('jai shree krishna', null, tz)));
    assert.ok(/^Good (morning|afternoon|evening)|^Hello/.test(router.smallTalkReply('hello', null, tz)), 'a plain hello must not be answered devotionally');
    for (const g of ['namaste', 'radhe radhe', 'hi kris', 'hello krishna']) assert.ok(router.isSmallTalk(g), g);
  });
  t('persona: small talk has the K.R.1.S voice, with no nautical leftovers', () => {
    const tz = { context: { tz: 'Asia/Calcutta' }, now: NOW };
    const all = [];
    for (let i = 0; i < 40; i++) for (const m of ['how are you', 'bye', 'ok', 'thanks', 'hi']) all.push(router.smallTalkReply(m + ' '.repeat(i), null, tz));
    const joined = all.join(' ');
    assert.ok(!/bridge|fair winds|safe watch|shipshape|steady as she goes|right you are|aye\b/i.test(joined), joined);
  });
  t('persona: both system prompts carry the Krishna framing and the respect rule', () => {
    const { systemPrompt: agentPrompt } = require('../src/agent');
    const prompts = [systemPrompt({ appName: 'Shuddha now', guideSnippets: [] }), agentPrompt({ appName: 'Shuddha now' })];
    for (const p of prompts) {
      assert.ok(/You are K\.R\.1\.S/.test(p));
      assert.ok(/inspired by Lord Krishna/.test(p));
      assert.ok(/Do not quote scripture or make religious claims unless the user raises the subject/.test(p));
      assert.ok(!/experienced ship|trustworthy ship|dress whites/i.test(p));
    }
  });

  console.log(`\nK.R.1.S: ${passed} passed, ${fails.length} failed`);
